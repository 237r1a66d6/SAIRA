...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    $sql = "SELECT * FROM partner_contacts ORDER BY created_at DESC";
    ...
    sendJsonResponse(true, $partners, 'Partner contacts retrieved', 200);
}
...
// Handle DELETE - Delete a partner contact
    ...
    preg_match('/\/partners\/(\d+)/', $path, $matches);
    
    if (!isset($matches[1])) {
        sendJsonResponse(false, [], 'Invalid partner ID', 400);
    }
    
    $partnerId = intval($matches[1]);
    
    $sql = "DELETE FROM partner_contacts WHERE id = ?";
    $stmt = $conn->prepare($sql);
...
// Handle PUT - Update partner contact status
    ...
    if (strpos($path, '/status') !== false) {
        ...
        $sql = "UPDATE partner_contacts SET status = ?, updated_at = NOW() WHERE id = ?";
        $stmt = $conn->prepare($sql);
