...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    try {
        $sql = "SELECT * FROM contact_submissions WHERE contact_type = 'educator' ORDER BY created_at DESC";
        $result = $conn->query($sql);
        
        if (!$result) {
            error_log("Educator contacts query failed: " . $conn->error);
        ...
        error_log("Educator contacts retrieved: " . count($contacts));
        sendJsonResponse(true, ['contacts' => $contacts], 'Educator contacts retrieved', 200);
    } catch (Exception $e) {
        error_log("Error fetching educator contacts: " . $e->getMessage());
...
// Handle DELETE - Delete an educator contact
    ...
    $educatorId = isset($_GET['id']) ? intval($_GET['id']) : 0;
    
    if (!$educatorId) {
        sendJsonResponse(false, [], 'Invalid educator ID', 400);
    }
    
    $sql = "DELETE FROM contact_submissions WHERE id = ? AND contact_type = 'educator'";
...
// Handle PUT - Update educator contact status
    ...
    if ($action === 'status') {
        ...
        $sql = "UPDATE contact_submissions SET status = ?, updated_at = NOW() WHERE id = ? AND contact_type = 'educator'";
        $stmt = $conn->prepare($sql);
